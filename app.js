(() => {
  ...
  function bindEvents() {
    ...
    $("staffGrantButton").addEventListener("click", handleStaffGrant);
  ...
  function updateDeductionPreview() {
    const rawAmount = Number($("staffAmount").value || 0);
    ...
    $("staffRewardBalance").textContent = String(balance + amount);
  }
  ...
  async function handleStaffDeduction(event) {
    ...
    if (!Number.isInteger(amount) || amount <= 0 || amount > 99) {
      showToast("請輸入 1～99 之間的扣除數量。", true);
      return;
    }
  ...
  async function handleStaffGrant(event) {
    event.preventDefault();
    if (!myProfile) return;
    ...
    if (!Number.isInteger(amount) || amount <= 0 || amount > 99) {
      showToast("請輸入 1～99 之間的獎勵數量。", true);
      return;
    }
    if (!pin) {
      showToast("請輸入工作人員密碼。", true);
    ...
    const confirmed = await askConfirm(
      "確認給予籌碼",
      `將給予「${reward.nickname}」${reward.amount} 枚獎勵籌碼，給予後共有 ${reward.balanceBefore + reward.amount} 枚。`,
      "確認給予"
    );
    if (!confirmed) return;

    const form = $("staffForm");
    const grantButton = $("staffGrantButton");
    setFormBusy(form, true);
    if (grantButton) setButtonBusy(grantButton, true, "給予中⋯");

    try {
      const deviceToken = getDeviceToken();
      if (!deviceToken) throw new Error("INVALID_DEVICE_TOKEN");

      const { data, error } = await db.rpc("grant_tokens", {
        p_device_token: deviceToken,
        p_amount: reward.amount,
        p_note: reward.note || null,
        p_staff_pin: reward.pin
      });
      ...
      const result = Array.isArray(data) ? data[0] : data;
      if (!result || !Number.isFinite(Number(result.token_balance))) {
        throw new Error("INVALID_GRANT_TOKENS_RESPONSE");
      }

      await refreshMyProfile();
      renderDashboard();
      closeDialogById("staffDialog");
      showToast(`已給予 ${reward.amount} 枚獎勵籌碼，目前共有 ${Number(result.token_balance)} 枚。`);
    } catch (error) {
      console.error("grant_tokens failed:", error);
      showToast(humanizeError(error), true);
    } finally {
      setFormBusy(form, false);
      if (grantButton) setButtonBusy(grantButton, false);
    }
  }
  ...
  function humanizeError(error) {
    ...
    const map = [
      ...
      ["INVALID_AMOUNT", "籌碼操作數量不正確。"],
      ["grant_tokens", "獎勵籌碼功能尚未完成 Supabase 設定。"],
      ["INVALID_GRANT_TOKENS_RESPONSE", "獎勵已送出，但資料庫沒有回傳正確餘額，請至 Supabase 檢查。"],
